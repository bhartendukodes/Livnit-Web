"""
CPU-compatible oriented IoU loss functions.
Uses pure PyTorch - no CUDA required.
"""
import torch
import numpy as np
from .box_intersection_2d_cpu import oriented_box_intersection_2d
from .min_enclosing_box import smallest_bounding_box


def box2corners_th(box: torch.Tensor) -> torch.Tensor:
    """Convert box coordinate to corners.

    Args:
        box: (B, N, 5) with x, y, w, h, alpha

    Returns:
        corners: (B, N, 4, 2)
    """
    B = box.size()[0]
    x = box[..., 0:1]
    y = box[..., 1:2]
    w = box[..., 2:3]
    h = box[..., 3:4]
    alpha = box[..., 4:5]

    x4 = torch.tensor([0.5, -0.5, -0.5, 0.5], device=box.device, dtype=box.dtype).view(1, 1, 4)
    x4 = x4 * w
    y4 = torch.tensor([0.5, 0.5, -0.5, -0.5], device=box.device, dtype=box.dtype).view(1, 1, 4)
    y4 = y4 * h

    corners = torch.stack([x4, y4], dim=-1)

    sin = torch.sin(alpha)
    cos = torch.cos(alpha)
    row1 = torch.cat([cos, sin], dim=-1)
    row2 = torch.cat([-sin, cos], dim=-1)
    rot_T = torch.stack([row1, row2], dim=-2)

    rotated = torch.bmm(corners.view([-1, 4, 2]), rot_T.view([-1, 2, 2]))
    rotated = rotated.view([B, -1, 4, 2])
    rotated[..., 0] += x
    rotated[..., 1] += y

    return rotated


def cal_iou(box1: torch.Tensor, box2: torch.Tensor):
    """Calculate IoU.

    Args:
        box1: (B, N, 5)
        box2: (B, N, 5)

    Returns:
        iou: (B, N)
        corners1: (B, N, 4, 2)
        corners2: (B, N, 4, 2)
        U: (B, N) area1 + area2 - inter_area
    """
    corners1 = box2corners_th(box1)
    corners2 = box2corners_th(box2)
    inter_area, _ = oriented_box_intersection_2d(corners1, corners2)
    area1 = box1[:, :, 2] * box1[:, :, 3]
    area2 = box2[:, :, 2] * box2[:, :, 3]
    u = area1 + area2 - inter_area
    iou = inter_area / (u + 1e-8)
    return iou, corners1, corners2, u


def enclosing_box_aligned(corners1: torch.Tensor, corners2: torch.Tensor):
    """Calculate the smallest enclosing box (axis-aligned).

    Args:
        corners1: (B, N, 4, 2)
        corners2: (B, N, 4, 2)

    Returns:
        w: (B, N)
        h: (B, N)
    """
    x1_max = torch.max(corners1[..., 0], dim=2)[0]
    x1_min = torch.min(corners1[..., 0], dim=2)[0]
    y1_max = torch.max(corners1[..., 1], dim=2)[0]
    y1_min = torch.min(corners1[..., 1], dim=2)[0]

    x2_max = torch.max(corners2[..., 0], dim=2)[0]
    x2_min = torch.min(corners2[..., 0], dim=2)[0]
    y2_max = torch.max(corners2[..., 1], dim=2)[0]
    y2_min = torch.min(corners2[..., 1], dim=2)[0]

    x_max = torch.max(x1_max, x2_max)
    x_min = torch.min(x1_min, x2_min)
    y_max = torch.max(y1_max, y2_max)
    y_min = torch.min(y1_min, y2_min)

    w = x_max - x_min
    h = y_max - y_min
    return w, h


def enclosing_box(corners1: torch.Tensor, corners2: torch.Tensor, enclosing_type: str = "aligned"):
    """Get enclosing box dimensions."""
    if enclosing_type == "aligned":
        return enclosing_box_aligned(corners1, corners2)
    elif enclosing_type == "smallest":
        return smallest_bounding_box(torch.cat([corners1, corners2], dim=-2))
    else:
        return enclosing_box_aligned(corners1, corners2)


def cal_giou(box1: torch.Tensor, box2: torch.Tensor, enclosing_type: str = "aligned"):
    """Calculate GIoU loss.

    Args:
        box1: (B, N, 5) with x, y, w, h, alpha
        box2: (B, N, 5)
        enclosing_type: "aligned" or "smallest"

    Returns:
        giou_loss: (B, N)
        iou: (B, N)
    """
    iou, corners1, corners2, u = cal_iou(box1, box2)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    area_c = w * h
    giou_loss = 1.0 - iou + (area_c - u) / (area_c + 1e-8)
    return giou_loss, iou


def cal_diou(box1: torch.Tensor, box2: torch.Tensor, enclosing_type: str = "aligned"):
    """Calculate DIoU loss."""
    iou, corners1, corners2, u = cal_iou(box1, box2)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    c2 = w * w + h * h
    x_offset = box1[..., 0] - box2[..., 0]
    y_offset = box1[..., 1] - box2[..., 1]
    d2 = x_offset * x_offset + y_offset * y_offset
    diou_loss = 1.0 - iou + d2 / (c2 + 1e-8)
    return diou_loss, iou
