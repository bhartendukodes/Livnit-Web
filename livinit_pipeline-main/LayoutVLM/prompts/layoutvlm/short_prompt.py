...
MAKE SURE have included below the constraints needed to satisfy the layout criteria.
If there are multiple similar assets (e.g., two accent chairs), please make sure to specify the variable name and the index of the asset instance (e.g., accent_chair[0], accent_chair[1]) when specifying the constraints.
Otherwise, the variable index should be 0 (e.g., sofa[0], coffee_table[0], tv_stand[0], storage_unit[0]).

Example instructions to follow:
# Position the TV on top of the TV stand, directly opposite the sofa, both facing and aligned with it
solver.on_top_of(tv[0], tv_stand[0])
solver.align_with(tv_stand[0], sofa[0], angle=180)
solver.align_with(tv[0], sofa[0], angle=180)

# IMPORTANT: Avoid any overlap or collision between furniture and void spaces (like doors and windows) with a high-weight constraint
for asset in [sofa[0], coffee_table[0], tv_stand[0], accent_chair[0], accent_chair[1], storage_unit[0]]:
    for void in [void_0[0], void_door_1[0]]:
        solver.distance_constraint(asset, void, 0.5, None, weight=10)
""".format(
    base_class_definitions=BASE_CLASS_DEFINITIONS
)
    ...
    if short_version:
        ...
        if add_example:
            ...
            for i in range(len(sample_tasks)):
                ...
                in_context_examples += f"LAYOUT CRITERIA: {sample_tasks[i]}\nEXISTING SCENE PROGRAM:\n{sample_task_programs[i]}\nCONSTRAINT PROGRAM:\n{sample_constraint_programs[i]}"
