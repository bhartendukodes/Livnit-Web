...
from .constraints import (Constraint, against_wall, align_with,
                          bbox_overlap_loss, distance_constraint,
                          point_towards)
...
class GradSolver:
    ...
    def is_fixture(self, instance_id):
        return (
            instance_id.startswith("walls")
            or instance_id.startswith("fixed_point")
            or instance_id == "room_0"
        )
    ...
    def project_back_to_polygon(self, existing_constraints):
        ...
        with torch.no_grad():
            ...
            for instance_id, asset in self.solver_assets.items():
                ...
                while not all(
                    [
                        polygon_shapely.buffer(EPSILON).contains(Point(corners[i]))
                    ...
                    for i in range(corners.shape[0]):
                        ...
                        if not polygon_shapely.buffer(EPSILON).contains(point):
                            projected_point = polygon_shapely.exterior.interpolate(
                                polygon_shapely.exterior.project(point)
                            )
                            assert polygon_shapely.buffer(EPSILON).contains(
                                projected_point
                            )
                            translation = [
            ...
            for _, asset in self.solver_assets.items():
                if asset.optimize:
                    if asset.onCeiling:
                        asset.position[2] = torch.tensor(3, dtype=torch.float32).to(
                            self.device
                        )
                    else:
            ...
            def _project(polygon, points):
                ...
                for i in range(len(points)):
                    point = points[i]
                    if not polygon_shapely.buffer(EPSILON).contains(point):
                        points[i] = polygon_shapely.exterior.interpolate(
                            polygon_shapely.exterior.project(point)
                        )
                assert all(
                    [
                        polygon_shapely.buffer(EPSILON).contains(point)
                        for point in points
                    ]
                )
            ...
            for constraint, instance_ids in existing_constraints:
                if constraint.constraint_name == "on_top_of":
                    ...
                    if asset_above.optimize:
                        asset_above.position[-1] = (
                            asset_below.position[2]
                            + asset_below.size[2] / 2
                            + asset_above.size[2] / 2
                        )
                        new_point = _project(
                            asset_below.get_2dpolygon().cpu().numpy(),
                            [asset_above.position[:2].cpu().numpy()],
                        )
                        asset_above.position[:2] = torch.tensor(
                            new_point[0], dtype=torch.float32
                        ).to(self.device)
    ...
    def calc_loss(self, existing_constraints, new_constraints):
        ...
        def calculate_overlap_loss():
            nonoverlap_assets = [
                ...
                if not self.is_fixture(asset.id)
            ]
            giou_loss, iou = bbox_overlap_loss(
                nonoverlap_assets,
                only_consider_overlapping_assets=True,
                skipped_asset_pairs=self.on_top_of_assets,
                device=self.device,
            )
        ...
        def evaluate_existing_constraints(existing_constraints):
            ...
            for constraint, instance_ids in existing_constraints:
                assets = [
                    self.solver_assets[instance_id] for instance_id in instance_ids
                ]
                for asset in assets:
                    if isinstance(asset, torch.Tensor) and not asset.requires_grad:
                        raise RuntimeError(
                            f"Asset tensor {asset} does not require gradients."
                        )
        ...
        def evaluate_new_constraints(new_constraints):
            ...
            for constraint, instance_ids in new_constraints:
                ...
                if not constraint.evaluate(assets).requires_grad:
                    raise RuntimeError(
                        f"Constraint.evaluate {constraint.constraint_name} does not require gradients."
                    )
                try:
                    total_loss += constraint.evaluate(assets, device=self.device)
                except Exception as e:
                    print(
                        f"Error evaluating constraint {constraint.constraint_name} with instance_ids {instance_ids}: {e}"
                    )
                    import pdb
    ...
    def annealing_objective_function(
        self, params, bounds, existing_constraints, new_constraints, params_dict
    ):
        for instance_id, param_dict in params_dict.items():
            position_idx = param_dict["position"]
            rotation_idx = param_dict["rotation"]
            self.solver_assets[instance_id].position.data = torch.tensor(
                params[position_idx : position_idx + 3],
                dtype=torch.float32,
                device=self.device,
            )
            self.solver_assets[instance_id].rotation.data = torch.tensor(
                params[rotation_idx : rotation_idx + 2],
                dtype=torch.float32,
                device=self.device,
            )
        overlap_loss, existing_constraint_loss, new_constraint_loss = self.calc_loss(
            existing_constraints, new_constraints
        )
        loss = overlap_loss + existing_constraint_loss + new_constraint_loss
        return loss.item()

    def scipy_minimize_objective_function(
        self, params, bounds, existing_constraints, new_constraints, params_dict
    ):
        for instance_id, param_dict in params_dict.items():
            position_idx = param_dict["position"]
    ...
    def optimize(
        ...
        saved_intermediate_states = {"boundary": self.boundary, "solver_assets": []}
        progress_bar = tqdm(total=iterations, desc="optimization progress")
        visualize_grid(
            self.boundary,
            self.solver_assets,
            os.path.join(temp_dir, f"frame_{progress_bar.n:04d}.png"),
        )
        ...
        if self.solver_type == "minimize":
            ...
            for instance_id, asset in self.solver_assets.items():
                if asset.optimize and asset.position.requires_grad:
                    ...
                    initial_params.extend(
                        asset.position.cpu().detach().numpy().tolist()
                    )
                    sz = min(asset.size[:3])
                    bounds.extend(
                        [
                            (x_min + sz / 2, x_max - sz / 2),
                            (y_min + sz / 2, y_max - sz / 2),
                            (0 + asset.size[2] / 2, 10),
                        ]
                    )
                ...
                if asset.optimize and asset.rotation.requires_grad:
                    ...
                    initial_params.extend(
                        asset.rotation.cpu().detach().numpy().tolist()
                    )
                    bounds.extend([(-1, 1), (-1, 1)])
                    rotation_idx = len(initial_params) - 2
                    constraints.append(
                        NonlinearConstraint(
                            lambda x, idx=rotation_idx: x[idx] ** 2
                            + x[idx + 1] ** 2,  # sin²(θ) + cos²(θ)
                            lb=1.0,  # Must equal 1
                            ub=1.0,
                        )
                    )
            ...
            def progress_callback(xk, state):
                ...
                for instance_id, param_dict in params_dict.items():
                    ...
                    rotation_idx = param_dict["rotation"]
                    self.solver_assets[instance_id].position.data = torch.tensor(
                        xk[position_idx : position_idx + 3],
                        dtype=torch.float32,
                        device=self.device,
                    )
                    self.solver_assets[instance_id].rotation.data = torch.tensor(
                        xk[rotation_idx : rotation_idx + 2],
                        dtype=torch.float32,
                        device=self.device,
                    )
                progress_bar.update(1)
                ...
                print(
                    f"Step {progress_bar.n}, Loss: {state.fun}"
                )  # Output the current loss (f)
                return False
            ...
            for instance_id, param_dict in params_dict.items():
                ...
                rotation_idx = param_dict["rotation"]
                self.solver_assets[instance_id].position.data = torch.tensor(
                    result.x[position_idx : position_idx + 3],
                    dtype=torch.float32,
                    device=self.device,
                )
                self.solver_assets[instance_id].rotation.data = torch.tensor(
                    result.x[rotation_idx : rotation_idx + 2],
                    dtype=torch.float32,
                    device=self.device,
                )
                print(
                    f"Instance {instance_id}, position: {self.solver_assets[instance_id].position}, rotation: {self.solver_assets[instance_id].rotation}"
                )
            visualize_grid(
                self.boundary,
                self.solver_assets,
                os.path.join(temp_dir, f"frame_final.png"),
            )
            ...
            def progress_callback(x, f, context):
                ...
                for instance_id, param_dict in params_dict.items():
                    ...
                    rotation_idx = param_dict["rotation"]
                    self.solver_assets[instance_id].position.data = torch.tensor(
                        x[position_idx : position_idx + 3],
                        dtype=torch.float32,
                        device=self.device,
                    )
                    self.solver_assets[instance_id].rotation.data = torch.tensor(
                        x[rotation_idx : rotation_idx + 2],
                        dtype=torch.float32,
                        device=self.device,
                    )
                visualize_grid(
            ...
            def is_better_solution(losses, best_losses):
                (
                    best_overlap_loss,
                    best_existing_constraint_loss,
                    best_new_constraint_loss,
                ) = [
                    float(loss.item() if isinstance(loss, torch.Tensor) else loss)
                    for loss in best_losses
                ]
                overlap_loss, existing_constraint_loss, new_constraint_loss = [
                    float(loss.item() if isinstance(loss, torch.Tensor) else loss)
                    for loss in losses
                ]
                if best_overlap_loss < 1e-3 and overlap_loss < 1e-3:
                    return (existing_constraint_loss + new_constraint_loss) < (
                        best_existing_constraint_loss + best_new_constraint_loss
                    )
                return (
                    overlap_loss + existing_constraint_loss + new_constraint_loss
                ) < (
            ...
            for i in tqdm(range(iterations)):
                ...
                if (i == iterations - 1 or i % 10 == 0) and temp_dir:
                    ...
                    assets_dict = {
                        asset_id: asset_to_dict(asset)
                        for asset_id, asset in self.solver_assets.items()
                        if not asset_id.startswith("walls")
                    }
                ...
                (
                    ...
                    new_constraint_loss,
                ) = self.calc_loss(existing_constraints, new_constraints)
                ...
                if i % 10 == 0 and is_better_solution(
                    (overlap_loss, existing_constraint_loss, new_constraint_loss),
                    best_losses,
                ):
                    best_idx = i
                    best_losses = [
                        float(
                            overlap_loss.item()
                            if isinstance(overlap_loss, torch.Tensor)
                            else overlap_loss
                        ),
                        float(
                            existing_constraint_loss.item()
                            if isinstance(existing_constraint_loss, torch.Tensor)
                            else existing_constraint_loss
                        ),
                        float(
                            new_constraint_loss.item()
                            if isinstance(new_constraint_loss, torch.Tensor)
                            else new_constraint_loss
                        ),
                    ]
                    print("Best losses updated:", best_losses)
                ...
                for asset_id, asset in self.solver_assets.items():
                    if torch.isnan(asset.position).any():
                        ...
                        asset.position = torch.where(
                            torch.isnan(asset.position),
                            torch.zeros_like(asset.position),
                            asset.position,
                        )
                ...
                if i % 100 == 0 or i == iterations - 2:
                    ...
                    scheduler.step()
                    print(
                        f"Iteration {i}, Total Loss: {loss.item()}, Overlap Loss: {overlap_loss.item() if isinstance(overlap_loss, torch.Tensor) else overlap_loss}, Existing Constraint Loss: {existing_constraint_loss.item() if isinstance(existing_constraint_loss, torch.Tensor) else existing_constraint_loss}, New Constraint Loss: {new_constraint_loss.item() if isinstance(new_constraint_loss, torch.Tensor) else new_constraint_loss}"
                    )

            if best_solution is not None and torch.cuda.is_available():
                print(f"Found better solution at iteration {best_idx}")
                for instance_id in target_instance_ids:
                    self.solver_assets[instance_id].position.data = best_solution[
                        instance_id
                    ]["position"]
                    self.solver_assets[instance_id].rotation.data = best_solution[
                        instance_id
                    ]["rotation"]
            else:  # Get the final solution
                print("Used the final solution while running CPU")
            if output_gif_path:
                with imageio.get_writer(
                    output_gif_path, mode="I", duration=0.5
                ) as writer:
