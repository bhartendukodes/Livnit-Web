...
from utils.placement_utils import (get_random_placement,
                                   replace_z_rot_degree_to_rpy_radians)
...
class LayoutVLM:
    def __init__(
        ...
        mode="finetuned",
        visual_mark_mode="new_coord",
        ft_original_model_id=None,
        ft_model_checkpoint=None,
        convert_z_rot_degree_to_rpy_radians=True,
        max_place_remaining_retry=2,
        ...
        self.llm_slow = ChatOpenAI(
            model_name=gpt_4o_model_name, max_tokens=2048, temperature=0.2, top_p=0.1
        )
        self.llm_slow_mini = ChatOpenAI(
            model_name="gpt-4o-mini", max_tokens=2048, temperature=0.2, top_p=0.1
        )
        self.llm_slow_grouping = ChatOpenAI(
            model_name="gpt-4o", max_tokens=2048, temperature=0.2, top_p=0.1
        )
    ...
    def mark_image(self, visual_marks, input_path, output_path, image_size=(512, 512)):
        if self.visual_mark_mode == "grid":
            ...
            annotate_image_with_coordinates(
                input_path, visual_marks, output_path, default_color="red"
            )
        else:
            raise NotImplementedError(
                f"Visual mark mode {self.visual_mark_mode} not implemented"
            )

    def get_asset_groups(
        self, task, MAX_ATTEMPTS=5, save_dir=None, include_position_in_prompt=False
    ):
        ...
        if isinstance(layout_criteria, list):
            layout_criteria = "\n".join(layout_criteria)
    ...
    def prepare_finetuned_vlm_prompt(self, final_prompt, encoded_images=[]):
        conversation = []
        user_content = [{"type": "text", "text": final_prompt.replace("<image>", "")}]
        user_content.extend([{"type": "image"} for _ in range(len(encoded_images))])
        conversation.append({"role": "user", "content": user_content})
        image_list = [
            Image.open(image_path).convert("RGB") for image_path in encoded_images
        ]
        return conversation, image_list

    def get_constraint_program(
        self,
        final_prompt,
        current_scene_image_path_dict,
        current_group_asset_img_path_dict,
        program_save_path=None,
    ):
        image_paths = []
        if self.mode != "no_image":
            top_down_scene_image_path = current_scene_image_path_dict[
                "top_down_rendering"
            ]
            side_scene_image_path = current_scene_image_path_dict["side_rendering_45_3"]
            asset_images = [
                current_group_asset_img_path_dict[asset_name]
                for asset_name in current_group_asset_img_path_dict
            ]
            image_paths = [
                top_down_scene_image_path,
                side_scene_image_path,
            ] + asset_images
    ...
    def get_task_program(grouped_assets, task, verify_asset_var_name_to_count=None):
        ...
        for wall_idx in range(len(task["boundary"]["floor_vertices"])):
            size_str1 = "[{:.2f}, {:.2f}, {:.2f}]".format(
                floor_vertices[wall_idx][0],
                floor_vertices[wall_idx][1],
                floor_vertices[wall_idx][2],
            )
            size_str2 = "[{:.2f}, {:.2f}, {:.2f}]".format(
                floor_vertices[(wall_idx + 1) % num_walls][0],
                floor_vertices[(wall_idx + 1) % num_walls][1],
                floor_vertices[(wall_idx + 1) % num_walls][2],
            )
        ...
        # Find longest wall and shortest wall
        for wall_idx in range(len(task["boundary"]["floor_vertices"])):
            p1 = np.array(floor_vertices[wall_idx])
            p2 = np.array(floor_vertices[(wall_idx + 1) % num_walls])
            length = np.linalg.norm(p2 - p1)
            if wall_idx == 0:
                longest_wall_length = length
                shortest_wall_length = length
                longest_wall_idx = wall_idx
                shortest_wall_idx = wall_idx
            else:
                if length > longest_wall_length:
                    longest_wall_length = length
                    longest_wall_idx = wall_idx
                if length < shortest_wall_length:
                    shortest_wall_length = length
                    shortest_wall_idx = wall_idx
        program += f"\n# Longest wall is wall[{longest_wall_idx}] with length {longest_wall_length:.2f} meters\n"
        program += f"# Shortest wall is wall[{shortest_wall_idx}] with length {shortest_wall_length:.2f} meters\n"
        ...
        for instance_uid, asset in task["assets"].items():
            ...
            if asset_uid not in uid2asset.keys():
                uid2asset[asset_uid] = {"asset": asset, "count": 1}
            else:
        ...
        for asset_uid, value in uid2asset.items():
            ...
            if verify_asset_var_name_to_count is not None:
                assert (
                    value["count"]
                    == verify_asset_var_name_to_count[asset["asset_var_name"]]
                ), "value['count'] ({}) != verify_asset_var_name_to_count[asset['asset_var_name']] ({}) for {}".format(
                    value["count"],
                    verify_asset_var_name_to_count[asset["asset_var_name"]],
                    asset["asset_var_name"],
                )
            program += (
                f"{asset['asset_var_name']} = Assets("
        ...
        for instance_uid, asset in task["assets"].items():
            ...
            # Ungroup voids to ensure they have individual sizes and definitions
            if "void" in asset_uid or asset.get("category") == "void":
                asset_uid = instance_uid.replace("-", "_")
                asset["asset_var_name"] = asset_uid
        ...
        for asset_uid, value in uid2asset.items():
            ...
            # Generate placements with specific position/rotation/optimize flags
            instances_str_list = []
            # Find all instances for this asset_uid
            matching_assets = [
                a
                for uid, a in task["assets"].items()
                if a.get("asset_var_name") == asset_uid and uid in grouped_assets
            ]
            matching_assets.sort(key=lambda x: x.get("instance_idx", 0))

            for ma in matching_assets:
                if "placements" in ma and len(ma["placements"]) > 0:
                    p = ma["placements"][0]
                    pos = p.get("position")
                    rot = p.get("rotation", [0.0, 0.0, 0.0])
                    # Use the optimize flag from the asset, default to 1. Force 0 for voids.
                    opt = p.get("optimize", 1)
                    if "void" in asset_uid:
                        opt = 0

                    if pos:
                        if len(pos) == 2:
                            pos = list(pos) + [0.0]
                        pos_str = "[{:.2f}, {:.2f}, {:.2f}]".format(
                            pos[0], pos[1], pos[2]
                        )
                        rot_str = "[{:.2f}, {:.2f}, {:.2f}]".format(
                            rot[0], rot[1], rot[2]
                        )
                        instances_str_list.append(
                            f"AssetInstance(position={pos_str}, rotation={rot_str}, optimize={opt})"
                        )
                    else:
                        instances_str_list.append("AssetInstance()")
                else:
                    instances_str_list.append("AssetInstance()")

            placements_str = "[" + ", ".join(instances_str_list) + "]"
    ...
    def _solve_single_group(
        ...
        current_scene_image_path_dict = {}
        if include_image:
            from utils.render_subprocess import should_use_subprocess, render_existing_scene_subprocess
            if should_use_subprocess():
                render_existing_scene = render_existing_scene_subprocess
                reset_blender = lambda: None  # No-op, subprocess handles cleanup
            else:
            ...
            for _image in output_images:
                current_scene_image_path_dict[
                    os.path.basename(_image).split(".")[0]
                ] = _image
        ...
        _task["assets"] = {
            k: v
            for k, v in task["assets"].items()
            if (k in placed_assets.keys() or k in group_assets)
        }
        task_program_for_prompt = self.get_task_program(group_assets, _task)

        asset_placed_list = [
            asset_name.replace("-", "[") + "]" if "-" in asset_name else asset_name
            for asset_name in task["assets"].keys()
            if asset_name not in group_assets
        ]
        asset_be_placed_list = [
            asset_name.replace("-", "[") + "]" if "-" in asset_name else asset_name
        ...
        final_prompt = short_prompt.get_layout_prompt(
            task_program_for_prompt,
            {"layout_criteria": layout_criteria},
            self.numerical_value_only,
        )
        ...
        for attempt_idx in range(MAX_ATTEMPTS):
            ...
            if True:
                ...
                constraint_program = self.get_constraint_program(
                    final_prompt,
                    current_scene_image_path_dict,
                    current_group_asset_img_path_dict,
                ...
                # Load sanitized program produced by sanity_check
                sanitized_path = os.path.join(_save_dir, "filtered_sandbox_program.py")
                if os.path.exists(sanitized_path):
                    with open(sanitized_path, "r") as f:
                        constraint_program = f.read()
                ...
                placed_assets = self.sandbox.solve(
                    ...
                    constraint_program,
                    save_dir=_save_dir,
                    only_initialize=only_initialize,
                )
                break
            # except Exception as e:
            #     print("Retrying ...", e)
    ...
    def solve(self, original_task, MAX_ATTEMPTS=3):
        ...
        if self.mode == "one_shot":
            ...
            group_list = self.get_asset_groups(
                task, save_dir=self.save_dir, MAX_ATTEMPTS=MAX_ATTEMPTS
            )
            with open(self.save_dir + "/grouping.json", "w") as f:
                json.dump(group_list, f, indent=4)

            placed_assets = self.sandbox.export_layout(incomplete_scene=True)
            for group_idx, group in enumerate(group_list):
                ...
                _key_relations_between_assets = "\n".join(
                    group["key_relations_between_assets"]
                )
        ...
        while (
            len(unplaced_assets)
            and num_groups < 20
            and num_place_remaining_retry < self.max_place_remaining_retry
        ):
            print(f"Placing unplaced assets while not empty -- group {num_groups}")
            _save_dir = os.path.join(self.save_dir, f"group_{num_groups}")
            placed_assets = self._solve_single_group(
                task,
                task["layout_criteria"],
        ...
        results = self.sandbox.export_layout()
        ...
        if self.mode not in ["no_constraint", "finetuned"]:
            ...
            for group_idx in range(num_groups):
                if os.path.exists(f"{self.sandbox.save_dir}/group_{group_idx}/out.gif"):
                    gif_files.append(
                        f"{self.sandbox.save_dir}/group_{group_idx}/out.gif"
                    )
    ...
    def solve_manual_with_sandbox_program(self, task, constraint_program):
        matches = extract_python_program(constraint_program)
        if matches:
            constraint_program = matches[0]
        ...
        print(f"Placing unplaced assets -- group {num_groups}")
        _save_dir = os.path.join(self.save_dir, f"group_{num_groups}")
        self.sandbox.sanity_check(unplaced_assets, constraint_program)
        placed_assets = self.sandbox.solve(
            placed_assets,
            unplaced_assets,
            constraint_program,
        ...
        unplaced_assets = set(task["assets"].keys()) - set(placed_assets.keys())
        print("Placed assets: ", placed_assets.keys())
        print("Unplaced assets: ", unplaced_assets)
        ...
        if self.mode not in ["no_constraint", "finetuned"]:
            ...
            for gif_file in gif_files:
                gif = imageio.mimread(gif_file)  # Read all frames from the GIF
                all_frames.extend(gif)
            print("Total frames collected for final gif: ", len(all_frames))
            if len(all_frames) > 0:
                imageio.mimsave(f"{self.save_dir}/final.gif", all_frames)
    ...
    def filter_constraint(self, image_path, final_prompt, save_path=None):
        ...
        messages = [("system", "You are a coding agent.")]
        content = [{"type": "text", "text": final_prompt}]
