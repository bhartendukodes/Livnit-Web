...
from .constraints import ALL_CONSTRAINTS, Constraint
...
def generate_loops(constraints):
    ...
    for (constraint_type, key), args_list in grouped_by_type.items():
        ...
        if constraint_type == "locate_grid":
            for args in args_list:
                output_constraints.append(
                    f"solver.{constraint_type}({', '.join(args)}))"
                )
            continue
        ...
        if len(args_list) > 1:
            ...
            first_asset_indices = [
                int(re.search(r"\[(\d+)\]", args[0]).group(1)) for args in args_list
            ]
            if (
                len(set(first_asset_indices)) == len(first_asset_indices)
                and len(first_asset_indices)
                == max(first_asset_indices) - min(first_asset_indices) + 1
            ):
                if all(args[1] == args_list[0][1] for args in args_list):
                    ...
                    loop_text += (
                        f"solver.{constraint_type}({key[0]}[i], {args_list[0][1]}"
                    )
                    if additional_args:
            ...
            second_asset_indices = [
                int(re.search(r"\[(\d+)\]", args[1]).group(1))
                for args in args_list
                if re.search(r"\[(\d+)\]", args[1])
            ]
            if (
                second_asset_indices
                and len(set(second_asset_indices)) == len(second_asset_indices)
                and len(second_asset_indices)
                == max(second_asset_indices) - min(second_asset_indices) + 1
            ):
                if all(args[0] == args_list[0][0] for args in args_list):
                    ...
                    loop_text += (
                        f"solver.{constraint_type}({args_list[0][0]}, {key[1]}[i]"
                    )
                    if additional_args:
                        loop_text += f", {', '.join(additional_args)}"
                    loop_text += ")"
        ...
        for args in args_list:
            ...
            output_constraints.append(
                f"solver.{constraint_type}({', '.join(cleaned_args)})"
            )
            new_group[cleaned_args[0].split("[")[0]].append(
                f"solver.{constraint_type}({', '.join(cleaned_args)})"
            )
...
class Wall:
    def __init__(self, wall_id, vertices, device=None):
        ...
        self.position = torch.tensor(
            [
                (self.corner1[0] + self.corner2[0]) / 2,
                (self.corner1[1] + self.corner2[1]) / 2,
                0,
            ],
            dtype=torch.float32,
            requires_grad=False,
        ...
        self.rotation = torch.tensor(
            [0, 0], dtype=torch.float32, requires_grad=False, device=self.device
        )
        # this is only for distance constraint?
        self.size = None
        self.dimension = torch.tensor(
            [
                abs(self.corner1[0] - self.corner2[0]),
    ...
    def get_2dvector(self, add_radian=0):
        ...
        if add_radian:
            ...
            rotation_matrix = torch.tensor(
                [[cos_theta, -sin_theta], [sin_theta, cos_theta]], dtype=torch.float32
            ).to(self.device)
...
class AssetInstance:
    def __init__(
        self, id, position, rotation, size, onCeiling=False, optimize=1, device=None
    ):
        self.id = id
        self.size = size
        ...
        # rotation is given in pipeline convention: 0=-Y, π/2=+X
        # convert to internal convention: 0=+X, π/2=+Y (standard math)
        # internal_θ = pipeline_θ - π/2
        internal_rz = rotation[-1] - math.pi / 2
        # add epsilon to avoid vanishing gradient
        internal_rz += math.pi / 1000
        self.rotation = torch.nn.Parameter(
            torch.tensor(
                [math.cos(internal_rz), math.sin(internal_rz)],
                dtype=torch.float32,
                device=self.device,
            ),
            requires_grad=(self.optimize > 0),
        ...
        # Ensure size is a list or tuple of at least 2 elements before converting to tensor
        if isinstance(size, (int, float)):
            size = [size, size, size]
        elif len(size) == 1:
            size = [size[0], size[0], size[0]]
        elif len(size) == 2:
            size = [size[0], size[1], 0.0]  # Default height to 0 if missing

        self.dimensions = torch.tensor(
            size, dtype=torch.float32, requires_grad=False
        ).to(self.device)
    ...
    def get_theta(self, use_degree=False):
        # internal convention: 0=+X, π/2=+Y
        internal_radian = math.atan2(self.rotation[1].item(), self.rotation[0].item())
        # convert to pipeline convention: 0=-Y, π/2=+X
        # pipeline_θ = internal_θ + π/2
        radian = internal_radian + math.pi / 2
        if use_degree:
    ...
    def get_2dvector(self, add_radian=0):
        ...
        if add_radian:
            # Assuming add_degree is the angle to add in degrees
            theta = torch.tensor(
                add_radian, dtype=torch.float32
    ...
    def get_2dpolygon(self):
        ...
        # Ensure dimensions has at least 2 elements
        if self.dimensions.dim() == 0:
            # Fallback if somehow it's still 0-dim, though __init__ should prevent this
            dims = [self.dimensions.item(), self.dimensions.item()]
        else:
        ...
        global_corners = (
            torch.matmul(local_corners, rotation_matrix) + self.position[:2]
        )
...
class Scene:
    def __init__(self, boundary, assets, full_assets, scene_id):
        ...
        for idx in range(num_walls):
            vertices = np.array(
                [boundary[idx], boundary[(idx + 1) % num_walls]]
            ).astype(np.float32)
        ...
        with torch.no_grad():
            ...
            for i in range(num_walls):
                ...
                for j in range(num_assets):
                    ...
                    constraint_value = constraint.evaluate(
                        [asset, wall_asset], device=device
                    ).item()
                    if debug:
                        print(
                            "Wall constraint value between asset {} and wall {}: {}".format(
                                asset.id, wall_asset.id, constraint_value
                            )
                        )
            ...
            for i in range(num_assets):
                ...
                for j in range(i + 1, num_assets):
                    ...
                    if (
                        point_constraint.evaluate([assets[i], assets[j]]).item()
                        < EPSILON
                    ):
                    ...
                    if (
                        point_constraint.evaluate([assets[j], assets[i]]).item()
                        < EPSILON
                    ):
                    ...
                    moderate_distance_away_constraint = ALL_CONSTRAINTS[
                        "moderate_distance"
                    ]
                    if (
                        close_to_constraint.evaluate([assets[i], assets[j]]).item()
                        < EPSILON
                    ):
                        ...
                        moderate_distance_away_constraint.evaluate(
                            [assets[i], assets[j]]
                        ).item()
                        < EPSILON
                    ):
                    ...
                    aligned_with_value = aligned_constraint.evaluate(
                        [assets[i], assets[j]]
                    ).item()
                    if debug:
                        print(
                            "Aligned with constraint value between asset {} and asset {}: {}".format(
                                assets[i].id, assets[j].id, aligned_with_value
                            )
                        )
                ...
                if i not in orientation_dict.keys():
                    ...
                    for w in range(num_walls):
                        ...
                        if (
                            aligned_constraint.evaluate([assets[i], wall_asset]).item()
                            == 0.0
                        ):
                    ...
                    for w in range(len(self.full_assets)):
                        ...
                        # print(aligned_constraint.evaluate([assets[i], wall_asset]).item())
                        if (
                            aligned_constraint.evaluate(
                                [assets[i], self.full_assets[w]]
                            ).item()
                            == 0.0
                            and asset_name_i != asset_name_w
                        ):
            ...
            for i in range(num_assets):
                ...
                for j in range(len(self.full_assets)):
                    ...
                    i_on_top_of_j = on_constraint.evaluate(
                        [assets[i], self.full_assets[j]]
                    ).item()
                    j_on_top_of_i = on_constraint.evaluate(
                        [self.full_assets[j], assets[i]]
                    ).item()
                    if debug:
                        print(
                            "On top of constraint value between asset {} and asset {}: {}".format(
                                assets[i].id, self.full_assets[j].id, i_on_top_of_j
                            )
                        )
                        print(
                            "On top of constraint value between asset {} and asset {}: {}".format(
                                self.full_assets[j].id, assets[i].id, j_on_top_of_i
                            )
                        )
    ...
    def extract_constraints_v2(
        self, visual_mark_instances, debug=False, use_rotation_for_optimization=False
    ):
        """
        ...
        moderate_distance_away_constraint = ALL_CONSTRAINTS[
            "moderate_distance_deterministic"
        ]
        aligned_constraint = ALL_CONSTRAINTS["align_with"]
        ...
        with torch.no_grad():
            ...
            for i in range(num_assets):
                ...
                for j in range(num_walls):
                    ...
                    constraint_value = wall_constraint.evaluate(
                        [asset_i, wall_asset]
                    ).item()
                    if debug:
                        print(
                            "Wall constraint value between asset {} and wall {}: {}".format(
                                asset_i.id, wall_asset.id, constraint_value
                            )
                        )
                ...
                if visual_mark_instances != []:
                    ...
                    for visual_mark_instance in visual_mark_instances:
                        # distance
                        close_to_constraint = ALL_CONSTRAINTS["close_to_deterministic"]
                        constraint_value, distance = close_to_constraint.evaluate(
                            [asset_i, visual_mark_instance]
                        )
                    ...
                    if visual_mark_to_distance:
                        visual_mark_instance = min(
                            visual_mark_to_distance, key=visual_mark_to_distance.get
                        )
                ...
                for j in range(num_assets):
                    ...
                    # distance
                    if (
                        close_to_constraint.evaluate([assets[i], assets[j]])[0].item()
                        < EPSILON
                    ):
                    ...
                    on_top_of_value = on_constraint.evaluate(
                        [assets[i], assets[j]]
                    ).item()
                    if debug:
                        print(
                            "On top of constraint value between asset {} and asset {}: {}".format(
                                assets[i].id, assets[j].id, on_top_of_value
                            )
                        )
            ...
            for i in range(num_assets):
                ...
                for k in range(len(self.full_assets)):
                    ...
                    if i not in orientation_dict and asset_k.id not in current_id_list:
                        ...
                        if (
                            point_constraint.evaluate([asset_i, asset_k]).item()
                            < EPSILON
                        ):
                    ...
                    # add on top of constraints between assets in different groups
                        ...
                        # print(asset_k)
                        if (
                            on_constraint.evaluate([asset_i, asset_k]).item()
                            < ON_TOP_OF_EPSILON
                        ):
                            program_str += f"solver.on_top_of({asset_name_i}[{asset_idx_i}], {asset_name_k}[{asset_idx_k}])\n"
                        elif (
                            on_constraint.evaluate([asset_k, asset_i]).item()
                            < ON_TOP_OF_EPSILON
                        ):
        ...
        with torch.no_grad():
            ...
            for i in range(num_assets):
                ...
                for j in range(num_walls):
                    ...
                    if constraint_value < WALL_EPSILON:
                        full_constraints[(i, "wall")][
                            "against_wall"
                        ] = f"solver.against_wall({asset_name_i}[{asset_idx_i}], {wall_asset.id})\n"
                        program_str += f"solver.against_wall({asset_name_i}[{asset_idx_i}], {wall_asset.id})\n"
                ...
                for j in range(num_assets):
                    ...
                    if (
                        ...
                        full_constraints[(i, j)][
                            "point_towards"
                        ] = f"solver.point_towards({asset_name_i}[{asset_idx_i}], {asset_name_j}[{asset_idx_j}])\n"
                        program_str += f"solver.point_towards({asset_name_i}[{asset_idx_i}], {asset_name_j}[{asset_idx_j}])\n"
                        continue
                    ...
                    if aligned_with_value < EPSILON:
                        ...
                        full_constraints[(i, j)][
                            "align_with"
                        ] = f"solver.align_with({asset_name_i}[{asset_idx_i}], {asset_name_j}[{asset_idx_j}])\n"
                        program_str += f"solver.align_with({asset_name_i}[{asset_idx_i}], {asset_name_j}[{asset_idx_j}])\n"
                        continue
                    ...
                    if on_top_of_value < ON_TOP_OF_EPSILON:
                        full_constraints[(i, j)][
                            "on_top_of"
                        ] = f"solver.on_top_of({asset_name_i}[{asset_idx_i}], {asset_name_j}[{asset_idx_j}])\n"
                        program_str += f"solver.on_top_of({asset_name_i}[{asset_idx_i}], {asset_name_j}[{asset_idx_j}])\n"
            ...
            for idx, constraints in full_constraints.items():
                for constraint, constraint_str in constraints.items():
                    ...
                    if j == "wall":
                        ...
                        new_constraints[(asset_i.id, "wall")][constraint] = (
                            position_str + constraint_str
                        )
                    else:
                        asset_i = self.assets[i]
                        ...
                        new_constraints[(asset_i.id, asset_j.id)][constraint] = (
                            position_str + constraint_str
                        )
            # # ### constraints between floor grid and assets
            # for i in range(num_assets):
